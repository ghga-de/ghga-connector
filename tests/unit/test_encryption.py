...
def make_encryptor(
    private_key, file_size: int, part_size: int | None = None
) -> Crypt4GHEncryptor:
    """Create a Crypt4GHEncryptor with an optional part_size (defaults to single-part)."""
...
def test_process_file_total_bytes_match_encrypted_size(private_key):
    ...
    file_data = os.urandom(2 * SEGMENT_SIZE + 100)
...
def test_process_file_ciphertext_size_matches_expected(private_key):
    ...
    file_data = os.urandom(3 * SEGMENT_SIZE + 77)
    encryptor = make_encryptor(private_key, file_size=len(file_data))
...
def test_process_file_unencrypted_checksum_matches_input(private_key):
    ...
    file_data = os.urandom(SEGMENT_SIZE + 200)
    encryptor = make_encryptor(private_key, file_size=len(file_data))
    run_process_file(encryptor, file_data)
    assert (
...
def test_process_file_encrypted_checksum_count_matches_part_count(private_key):
    ...
    file_data = os.urandom(4 * SEGMENT_SIZE)
    # Use CIPHER_SEGMENT_SIZE as part_size to force multiple parts
    encryptor = make_encryptor(
        private_key, file_size=len(file_data), part_size=CIPHER_SEGMENT_SIZE
    )
    parts = run_process_file(encryptor, file_data)
