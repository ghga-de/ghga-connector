...
class BatchUploader:
    """A class to manage multiple file uploads for a single upload work package."""

    def __init__(self, *, upload_client: UploadClient, config: Config):
        """Initialize the Batch Uploader"""
        self._upload_client = upload_client
        self._config = config

    async def upload_files(
        self, *, file_info_list: list[FileInfoForUpload], my_private_key: SecretBytes
    ):
        """Upload all files in the provided list of file paths"""
        self._file_info_list = file_info_list

        CLIMessageDisplay.display(
            f"Starting batch upload of {len(self._file_info_list)} files"
        )
